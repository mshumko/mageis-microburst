...
for i, (j, Ek) in enumerate(zip(np.transpose(d['FEDU'][:, PA_PLOT_IND, :]), d['e_energy_ch'])):
...
for (i, ii) in enumerate(range(START_ENERGY_IND, END_ENERGY_IND)):
    plt.scatter(d['Epoch_Ele'], d['FEDU_PSD'][:, ii], 
                label='{}'.format(round(d['e_energy_ch'][ii]/1000)), c=colors[i])
plt.legend(loc=1, bbox_to_anchor=(1.15, 1.05), title='Energy [keV]')
plt.yscale('log')
plt.xlabel('UTC')
plt.ylabel(r'PSD $c^3/(cm \ MeV)^3$')
plt.xlim(START_TIME, END_TIME)

plt.ylim(bottom=10**-2, top=10)
plt.title(r'2017-03-31 | RBSP-A | HOPE Electrons | $\alpha_L$' + 
            ' = {}'.format(d['PITCH_ANGLE'][PA_PLOT_IND]))
plt.savefig(('/home/mike/Dropbox/0_grad_work/mageis_microburst/plots/'
            'HOPE/20170331_hope_psd_a_{}.png'.format(int(d['PITCH_ANGLE'][PA_PLOT_IND]))))
plt.clear()